...
User invocation in Codex uses `$hstack-poteto-mode` or `$hstack-setup-pstack`. The portable identifier is the full `hstack-` name. A host may show an additional plugin namespace, but colon syntax is not assumed across hosts. Bare `poteto-mode` and `setup-pstack` belong to upstream pstack and do not select hstack.

Once hstack is selected, resolve every bundled workflow through [this package's skill map](../../SKILL-MAP.json). Its paths are relative to this package's root. The map accepts inherited labels such as `how` and points to unique installed files such as `skills/hstack-how/SKILL.md`. Read that exact file. Never search a global catalog for an unprefixed substitute, including the separate `pstack-plugin` edition. If a mapped file is missing, report the installation fault and repair it within the authorized scope instead of silently switching stacks. Upstream prose that names canonical `pstack/skills/` files means the corresponding mapped package file; do not fetch upstream `main` to replace this selected workflow. Preserve actual source URLs when inspecting upstream evidence.

The generated Codex package exposes `hstack-poteto-mode` and `hstack-setup-pstack` for native catalog selection, with descriptions scoped to hstack requests. Other workflows preserve explicit invocation policy. Read a selected sibling directly even if the initial catalog omits it. Relative links resolve from the file containing the link. Cursor's `mode`, `reminder`, and `.mdc` features do not exist here; continue the selected workflow within the current task without claiming a global sticky mode. Catalog visibility does not change tool permissions or make every workflow run automatically.
...
For `hstack-poteto-agent`, give the delegate an explicit instruction to read the mapped `poteto-mode` workflow and this adapter before working. For `hstack-comment-sicko`, read the mapped agent file and carry its report-only role into the task prompt. Other reviewers receive the applicable reference prompt, bounded scope, evidence paths, and ownership. Delegate prompts name hstack and provide absolute paths to the selected workflow, agent instructions, and adapter because detached context may not include them. Use the actual available Codex collaboration tools; unavailable named-agent support never justifies selecting an upstream `poteto-agent` or `Comment Sicko`.
