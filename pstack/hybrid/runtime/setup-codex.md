...
description: Configure hstack model roles in Codex when the user requests hstack setup or hstack-setup-pstack. Preserve existing model choices unless a change is requested.
---

# Set up hstack in Codex

Read [the Codex runtime adapter](../../hybrid/runtime/codex.md). This native setup replaces the upstream Cursor rule workflow. The unique `hstack-setup-pstack` command preserves the existing `pstack-models.json` preference path so model choices continue to work. Bare `setup-pstack` belongs to upstream pstack.
...
4. Write the complete JSON with `version`, `default`, and `roles` to `~/.codex/pstack-models.json`. Use exactly the role keys from the bundled defaults. Read it back, validate every role and panel, and report the effective changes. This file is loaded by hstack when invoked; it does not change the model of unrelated Codex tasks.
5. Tell the user to open a new Codex task after installation and invoke `$hstack-poteto-mode` with their engineering request. They can invoke `$hstack-setup-pstack` later to change routing. Project-specific verification harnesses are created when a real project task calls for one.
