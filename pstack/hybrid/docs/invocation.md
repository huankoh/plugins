...
| Codex | `$hstack-poteto-mode` | `$poteto-mode` |
...
The portable skill name uses hyphens. A host may add its own plugin namespace in the UI, but `hstack:poteto-mode` is not the cross-host command contract. Cursor documents `/skill-name` and requires the skill name to match its folder. [Cursor skills](https://cursor.com/docs/skills)
